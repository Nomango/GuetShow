...
export function swiperAnimateCache() {
  const allBoxes = window.document.documentElement.querySelectorAll(
    ".ani"
  ) as any;
...
export function swiperAnimate(a: any) {
  clearSwiperAnimate();
  const b = a.slides[a.activeIndex].querySelectorAll(".ani") as any;
  for (let i = 0; i < b.length; i++) {
    b[i].style.visibility = "visible";
    const effect = b[i].attributes["swiper-animate-effect"]
      ? "animate__" + b[i].attributes["swiper-animate-effect"].value
      : "";
...
export function clearSwiperAnimate() {
  ...
  for (let i = 0; i < allBoxes.length; i++) {
    ...
    const effectValue =
      "animate__" + allBoxes[i].attributes["swiper-animate-effect"].value;
